...
alt.on('keyup', (key) => {
    if ( key == 72 && alt.Player.local.vehicle && native.getEntityModel(alt.Player.local.vehicle.scriptID) == native.getHashKey('flatbed') ) { 
        alt.emitServer('nvrpg:flatbed:gettowedvehicleslist');
    }  
});

alt.onServer('nvrpg:flatbed:sendtowedvehicleslist', (list) => {
    if (list == '') {
        alt.emitServer('nvrpg:flatbed:getvehicleslist');
    } 
    else 
    {
        let found = false
        list.forEach(tow => {
            if ( tow.flatbed && tow.flatbed == alt.Player.local.vehicle.scriptID ) {
                ...
                alt.emitServer('nvrpg:flatbed:removetow', tow);
                found = true
            }
        });
...
alt.onServer('nvrpg:flatbed:sendvehicleslist', (list) => {
    ...
    list.forEach(veh => {
        if (veh.scriptID !=0) {
            let vehcoords = native.getEntityCoords(veh.scriptID, false)
            ...
            if ( !found && ( dist <= 10.0 && dist >= 8.0 && dist != 0 ) ) {
                ...
                alt.emitServer('nvrpg:flatbed:addtow', alt.Player.local.vehicle.scriptID, veh.scriptID);
                found = true
            }
        }
    });
