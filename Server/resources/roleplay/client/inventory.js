...
function GiveItem(itemname, itemAmount, fromContainer) {
    if (!canInteract) return;
    lastInteract = Date.now()
    alt.emitServer("Server:Inventory:GiveItem", itemname, parseInt(itemAmount), fromContainer);
}
...
alt.onServer('Client:Inventory:PlayEffect', (effectName, duration, looped) => {
    let interval = alt.setInterval(() => {
            alt.clearInterval(interval);
