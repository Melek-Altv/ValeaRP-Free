...
alt.onServer("Client:HUD:CreateCEF", () => {
    ...
    adminMenuBrowser.on("Client:AdminMenu:GetPlayerMeta", (charId, tarray) => {
        adminmenu_latestonlineactionarray = tarray;
        alt.emitServer("Server:AdminMenu:GetPlayer", "GetPlayerMeta", charId, "none");
    });

    adminMenuBrowser.on("Client:AdminMenu:SetMeta", (charId, selectedfield) => {
        alt.emitServer("Server:AdminMenu:GetPlayer", "SetMeta", charId, selectedfield);
    });
