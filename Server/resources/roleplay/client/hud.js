...
let MinijobLKWdriverCefOpened = false;
...
let changeNameCefOpened = false;
let isPlayerUsingMegaphone = false;
let changeKeyCefOpened = false;
let lastInteract = 0;
let craftingCefOpened = false;
let isProgressActive = false;
let jailMenuCefOpened = false;

alt.onServer("Client:HUD:CreateCEF", (hunger, thirst, health, armor) => {
    if (hudBrowser == null) {
        hudBrowser = new alt.WebView("http://resource/client/cef/hud/index.html");

        hudBrowser.on("Client:HUD:cefIsReady", () => {
            alt.setTimeout(function() {
                hudBrowser.emit("CEF:HUD:updateDesireHUD", hunger, thirst, health, armor);
                browserReady = true;
            }, 1000);
        });
        ...
        hudBrowser.on("Client:ChangeKeyCEF:destroyCEF", () => {
            ...
            alt.toggleGameControls(true);
            hudBrowser.unfocus();
            changeKeyCefOpened = false;
        });

        hudBrowser.on("Client:ChangeKeyCEF:changeVehKey", (id) => {
            alt.emitServer("Server:ChangeKeyCEF:changeVehKey", parseInt(id));
        });

        hudBrowser.on("Client:ChangeKeyCEF:changeHouseKey", (id) => {
            alt.emitServer("Server:ChangeKeyCEF:changeHouseKey", parseInt(id));
        });

        hudBrowser.on("Client:Lifeinvader:close", () => {
            alt.emit("Client:HUD:setCefStatus", false);
            game.freezeEntityPosition(alt.Player.local.scriptID, false);
            alt.showCursor(false);
            alt.toggleGameControls(true);
        ...
        hudBrowser.on("Client:Lifeinvader:createAd", (title, text) => {
            alt.emitServer("Server:Lifeinvader:createAd", title, text);
        });
        ...
        hudBrowser.on("Client:Barber:UpdateHeadOverlays", (headoverlayarray) => {
            ...
            game.setPedHeadOverlay(alt.Player.local.scriptID, 0, parseInt(headoverlays[0][0]), parseFloat(headoverlays[1][0]));
            ...
            game.setPedHeadOverlay(alt.Player.local.scriptID, 3, parseInt(headoverlays[0][3]), parseFloat(headoverlays[1][3]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 4, parseInt(headoverlays[0][4]), parseFloat(headoverlays[1][4]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 5, parseInt(headoverlays[0][5]), parseFloat(headoverlays[1][5]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 6, parseInt(headoverlays[0][6]), parseFloat(headoverlays[1][6]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 7, parseInt(headoverlays[0][7]), parseFloat(headoverlays[1][7]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 8, parseInt(headoverlays[0][8]), parseFloat(headoverlays[1][8]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 9, parseInt(headoverlays[0][9]), parseFloat(headoverlays[1][9]));
            game.setPedHeadOverlay(alt.Player.local.scriptID, 10, parseInt(headoverlays[0][10]), parseFloat(headoverlays[1][10]));
		...
		hudBrowser.on("Client:Surgery:UpdateHeadOverlays", (headoverlayarray, facefeaturesarray) => {
            let headoverlays = JSON.parse(headoverlayarray);
        ...
        hudBrowser.on("Client:Surgery:finishSurgery", (headoverlayarray1, headoverlayarray2, headoverlayarray3, facefeature) => {
            if(lastInteract + 500 > Date.now()) return;
            lastInteract = Date.now();
            
            alt.emitServer("Server:Surgery:finishSurgery", headoverlayarray1, headoverlayarray2, headoverlayarray3, facefeature);
            closeSurgeryCEF();
        });

		hudBrowser.on("Client:Surgery:destroySurgeryCEF", () => {
            closeSurgeryCEF();
        });
        ...
        hudBrowser.on("Client:VehicleShop:TestVehicle", (shopId, hash) => {
            if(lastInteract + 500 > Date.now()) return;
            lastInteract = Date.now();

            alt.emitServer("Server:VehicleShop:TestVehicle", parseInt(shopId), hash);
        });

        hudBrowser.on("Client:Garage:DoAction", (garageid, action, vehid, isHouseGarage) => {
            if(lastInteract + 500 > Date.now()) return;
            lastInteract = Date.now();

            alt.emitServer("Server:Garage:DoAction", parseInt(garageid), action, parseInt(vehid), isHouseGarage);
        });
        ...
        // Crafting
        hudBrowser.on("Client:CraftingStation:destroyCEF", () => {
            // alt.emitServer("Server:CEF:setCefStatus", false);
            alt.emit("Client:HUD:setCefStatus", false);
            game.freezeEntityPosition(alt.Player.local.scriptID, false);
            alt.showCursor(false);
            alt.toggleGameControls(true);
            hudBrowser.unfocus();
            craftingCefOpened = false;
        });

        hudBrowser.on("Client:CraftingStation:produceItem", (craftingStationId, recipeId, amount) => {
            alt.emitServer("Server:CraftingStation:produceItem", parseInt(craftingStationId), parseInt(recipeId), parseInt(amount));
        });
        ...
        hudBrowser.on("Client:ClothesShop:setClothes", (componentId, drawableId, textureId) => {
            ...
            //alt.emit("Server:Character:SetCharacterCorrectTorso", alt.Player.local.scriptID, 15);
        });
        ...
        hudBrowser.on("Client:MinijobLKWdriver:destroyCEF", () => {
            closeMinijobLKWdriverCEF();
        });

        hudBrowser.on("Client:MinijobLKWdriver:StartJob", (routeId) => {
            alt.emitServer("Server:Minijoblkw:StartJob", parseInt(routeId));
        });
        ...
        //Progress Bar
		hudBrowser.on("Client:Progressbar:stop", () => {
        ...
        /* ChangeVehOwner */
        hudBrowser.on("Client:Vehicle:changeVehOwner", (targetname) => {
            alt.emitServer("Server:Vehicle:changeVehOwner", (targetname));
        });
		...
		// Name Ändern 
        hudBrowser.on("Client:Townhall:changePlayerName", (name) => {
            alt.emitServer("Server:Townhall:ChangePlayerName", (name));
        });

        hudBrowser.on("Client:Townhall:closeChangePlayerNameHUD", () => {
            alt.emit("Client:HUD:setCefStatus", false);
            game.freezeEntityPosition(alt.Player.local.scriptID, false);
            alt.showCursor(false);
            alt.toggleGameControls(true);
            hudBrowser.unfocus();
            changeNameCefOpened = false;
        });
		
        // Jail Menu
        hudBrowser.on("Client:Jail:jailplayer", (jailTime) => {
            alt.emitServer("Server:Raycast:jailPlayer", parseInt(jailTime));
        });

        hudBrowser.on("Client:Jail:closeJailMenuHUD", () => {
            alt.emit("Client:HUD:setCefStatus", false);
            game.freezeEntityPosition(alt.Player.local.scriptID, false);
            alt.showCursor(false);
            alt.toggleGameControls(true);
            hudBrowser.unfocus();
            jailMenuCefOpened = false;
        });
        ...
        hudBrowser.on("Client:Smartphone:setAnonymCallEnabled", (isEnabled) => {
            alt.emitServer("Server:Smartphone:setAnonymCallEnabled", isEnabled);
        });
        ...
        hudBrowser.on("Client:Smartphone:requestAnrufliste", () => {
            alt.emitServer("Server:Smartphone:requestAnrufliste");
        });
...
alt.onServer("Client:HUD:UpdateDesire", (hunger, thirst, health, armor) => {
    if (hudBrowser != null) {
        hudBrowser.emit("CEF:HUD:updateDesireHUD", hunger, thirst, health, armor);
    }
});

alt.onServer("Client:HUD:updateHUDPosInVeh", (state, fuel, maxfuel, km) => {
    if (hudBrowser != null) {
        hudBrowser.emit("CEF:HUD:updateHUDPosInVeh", state, fuel, maxfuel, km);
    }
});
...
alt.onServer("Client:Lifeinvader:openCEF", () => {
    if (hudBrowser == null && alt.Player.local.getMeta("IsCefOpen") == true && isLifeInvaderOpened == true) return;
    hudBrowser.emit("CEF:Lifeinvader:open");
    alt.emit("Client:HUD:setCefStatus", true);
    game.freezeEntityPosition(alt.Player.local.scriptID, true);
    alt.showCursor(true);
    alt.toggleGameControls(false);
    hudBrowser.focus();
    isLifeInvaderOpened = false;
});
...
alt.onServer("Client:License:openLicenseOverview", (name, pkw, lkw, bike, boat, fly, helicopter, passengertransport, weaponlicense) => {
    if (hudBrowser == null) return;
    hudBrowser.emit("CEF:License:openLicenseOverview", name, pkw, lkw, bike, boat, fly, helicopter, passengertransport, weaponlicense);
});
...
alt.onServer("Client:Surgery:surgeryCreateCEF", (headoverlayarray1, headoverlayarray2, headoverlayarray3, facefeature) => {
    if (hudBrowser != null && alt.Player.local.getMeta("IsCefOpen") == false && SurgeryCefOpened == false) {
		console.log(facefeature);
        hudBrowser.emit("CEF:Surgery:surgeryCEFBoxCreateCEF", headoverlayarray1, headoverlayarray2, headoverlayarray3, facefeature);
        alt.emit("Client:HUD:setCefStatus", true);
        game.freezeEntityPosition(alt.Player.local.scriptID, true);
        alt.showCursor(true);
        alt.toggleGameControls(false);
        hudBrowser.focus();
        SurgeryCefOpened = true;

        let surgeryInterval = alt.setInterval(() => {
            game.invalidateIdleCam();
            if (SurgeryCefOpened === false) {
                alt.clearInterval(surgeryInterval);
            }
        }, 5000);
    }
});

alt.onServer("Client:Garage:OpenGarage", (garageId, garagename, garageInArray, garageOutArray, isHouseGarage) => {
    if (hudBrowser != null && alt.Player.local.getMeta("IsCefOpen") == false && GarageCefOpened == false) {
        hudBrowser.emit("CEF:Garage:OpenGarage", garageId, garagename, garageInArray, garageOutArray, isHouseGarage);
        alt.emit("Client:HUD:setCefStatus", true);
        game.freezeEntityPosition(alt.Player.local.scriptID, true);
        alt.showCursor(true);
        alt.toggleGameControls(false);
...
alt.onServer("Client:CrafingStation:open", (stationId, recipeJson) => {
    if (hudBrowser == null || alt.Player.local.getSyncedMeta("IsCefOpen") == true || craftingCefOpened == true) return;
    hudBrowser.emit("CEF:CrafingStation:open", stationId, recipeJson);
    alt.emit("Client:HUD:setCefStatus", true);
    game.freezeEntityPosition(alt.Player.local.scriptID, true);
    alt.showCursor(true);
    alt.toggleGameControls(false);
    hudBrowser.focus();
    craftingCefOpened = true;
});
...
//Progress Bar
alt.onServer("Client:Progressbar:start", (time) => {
    if (hudBrowser == null && alt.Player.local.getMeta("IsCefOpen") == true && isProgressActive == true) return;
    hudBrowser.emit("CEF:Progressbar:start", time);
    alt.emit("Client:HUD:setCefStatus", true);
    game.freezeEntityPosition(alt.Player.local.scriptID, true);
    alt.toggleGameControls(false);
    hudBrowser.focus();
...
alt.onServer("Client:Vehicle:showChangeOwnerHUD", (vehName) => {
    if (hudBrowser != null && alt.Player.local.getMeta("IsCefOpen") == false && changeVehOwnerCefOpened == false) {
...
alt.onServer("Client:Townhall:showChangePlayerNameHUD", (name) => {
    if (hudBrowser != null && alt.Player.local.getMeta("IsCefOpen") == false && changeNameCefOpened == false) {
        hudBrowser.emit("CEF:Townhall:showChangePlayerNameHUD", name);
        alt.emit("Client:HUD:setCefStatus", true);
        game.freezeEntityPosition(alt.Player.local.scriptID, true);
        alt.showCursor(true);
        alt.toggleGameControls(false);
        hudBrowser.focus();
        changeNameCefOpened = true;
    }
});

alt.onServer("Client:Jail:showJailMenuHUD", (vehName) => {
    if (hudBrowser != null && alt.Player.local.getMeta("IsCefOpen") == false && jailMenuCefOpened == false) {
        hudBrowser.emit("CEF:Jail:showJailMenuHUD", vehName);
        alt.emit("Client:HUD:setCefStatus", true);
        game.freezeEntityPosition(alt.Player.local.scriptID, true);
        alt.showCursor(true);
        alt.toggleGameControls(false);
        hudBrowser.focus();
        jailMenuCefOpened = true;
    }
});
...
alt.onServer("Client:MinijobLKWdriver:openCEF", (routeArray) => {
    if (hudBrowser != null && alt.Player.local.getMeta("IsCefOpen") == false && MinijobLKWdriverCefOpened == false) {
        hudBrowser.emit("CEF:MinijobLKWdriver:openCEF", routeArray);
        alt.emit("Client:HUD:setCefStatus", true);
        game.freezeEntityPosition(alt.Player.local.scriptID, true);
        alt.showCursor(true);
        alt.toggleGameControls(false);
        hudBrowser.focus();
        MinijobLKWdriverCefOpened = true;
    }
});
...
alt.onServer("Client:Smartphone:setAnrufliste", (json) => {
    if (hudBrowser == null || !browserReady) return;
    hudBrowser.emit("CEF:Smartphone:setAllAnrufliste", json);
});
...
alt.onServer("Client:HUD:SetPlayerHUDVehicleInfos", (fuel, maxfuel, km) => {
    if (hudBrowser != null && alt.Player.local.vehicle != null) {
        hudBrowser.emit("CEF:HUD:SetPlayerHUDVehicleInfos", fuel, maxfuel, km);
    }
});
...
alt.onServer("Client:AnimationM(enuPage2:SetMenuItems", (itemArray) => {
    if (hudBrowser != null) {
...
alt.on('keydown', (key) => {
	...
	else if (key == 117 && Crosshair == false) { // 117 = F6
		hudBrowser.emit("Client:DrawCrosshair", true);
		Crosshair = true;
		return;
    }
	else if (key === 117 && Crosshair == true) { // = F6
        hudBrowser.emit("Client:DrawCrosshair", false);
		Crosshair = false;
		return;
    }
    // SALTYCHAT 
    else if (key === 'N'.charCodeAt(0) && alt.Player.local.getMeta("IsCefOpen") == false && alt.Player.local.getSyncedMeta("HasHandcuffs") == false && alt.Player.local.getSyncedMeta("HasRopeCuffs") == false) {
...
alt.on('keyup', (key) => {
    if (key == 'X'.charCodeAt(0)) {
        ...
        if (hudBrowser == null || !browserReady || isPlayerDead || alt.Player.local.getSyncedMeta("HasHandcuffs") == true || alt.Player.local.getSyncedMeta("HasRopeCuffs") == true) return;
        ...
        playAnimation("amb@code_human_wander_texting@male@base", "static", 49, -1);
        alt.showCursor(true);
        alt.emit("Client:HUD:setCefStatus", true);
        alt.emitServer("Server:Phone:Attachment", true);
        alt.toggleGameControls(false);
        hudBrowser.focus();
    } else if (key == 34) {
        ...
        alt.emit("Client:HUD:setCefStatus", false);
        alt.emitServer("Server:Phone:Attachment", false);
        alt.showCursor(false);
        alt.toggleGameControls(true);
        hudBrowser.unfocus();
    }  
...
function InterActionMenuDoAction(type, action) {
    if (selectedRaycastId != null && selectedRaycastId != 0 && type != "none") {
        ...
        if (type == "vehicle") {
            ...
            } else if (action == "vehInformationVehicle") {
                alt.emitServer("Server:Raycast:showVehicleInformations", vehicle);
            ...
            } else if (action == "toggleVehHood") {
                alt.emitServer("Server:Raycast:toggleVehicleHood", vehicle);
            } else if (action == "toggleSeatbelt") {
                alt.emitServer("Server:Raycast:toggleSeatbelt", vehicle);
            }
            vehicle = null;
        } else if (type == "player") {
            ...
            if (action == "playersupportId") {
                ...
                alt.emitServer("Server:Raycast:PlayerStabilisieren", playerRC);
                ...
                alt.emitServer("Server:Raycast:showJailMenuHUD", playerRC);
            } else if (action == "playerJail") {
            ...
            } else if (action == "showLicenses") {
                alt.emitServer("Server:Raycast:showLicenses", playerRC);    
...
let closeSurgeryCEF = function() {
    alt.emit("Client:HUD:setCefStatus", false);
    game.freezeEntityPosition(alt.Player.local.scriptID, false);
    alt.showCursor(false);
    alt.toggleGameControls(true);
    hudBrowser.unfocus();
    SurgeryCefOpened = false;
}
...
let closeClothesStorageCEF = function() {
    alt.emit("Server:ClothesShop:RequestCurrentSkin");
    alt.emit("Client:HUD:setCefStatus", false);
    game.freezeEntityPosition(alt.Player.local.scriptID, false);
    alt.toggleGameControls(true);
    hudBrowser.unfocus();
...
let closeMinijobLKWdriverCEF = function() {
    alt.emit("Client:HUD:setCefStatus", false);
    game.freezeEntityPosition(alt.Player.local.scriptID, false);
    alt.showCursor(false);
    alt.toggleGameControls(true);
    hudBrowser.unfocus();
    MinijobLKWdriverCefOpened = false;
}
...
let closeAllCEFs = function() {
    if (hudBrowser == null) return;
    if (identityCardApplyCEFopened || TuningMenuCefOpened || BankAccountManageFormOpened || ATMcefOpened || ShopCefOpened || BarberCefOpened || SurgeryCefOpened || GarageCefOpened || VehicleShopCefOpened || JobcenterCefOpened || FuelStationCefOpened || ClothesShopCefOpened || bankFactionATMCefOpened || GivePlayerBillCefOpened || FactionStorageCefOpened || RecievePlayerBillCefOpened || VehicleTrunkCefOpened || VehicleLicensingCefOpened || PlayerSearchInventoryCefOpened || GivePlayerLicenseCefOpened || MinijobBusdriverCefOpened || MinijobLKWdriverCefOpened || HotelRentCefOpened || HouseEntranceCefOpened || HouseManageCefOpened || TownhallHouseSelectorCefOpened || AnimationMenuCefOpened) {
...
alt.onServer("Client:ChangeKey:openCEF", (vehicles, houses) => {
    if (hudBrowser == null && changeKeyCefOpened) return;
    changeKeyCefOpened = true;
    hudBrowser.emit("CEF:ChangeKey:openCEF", vehicles, houses);
    alt.showCursor(true);
    alt.toggleGameControls(false);
    hudBrowser.focus();
});
...
alt.onServer("Client:HUD:setCefStatus", (state) => {
    if (state == alt.Player.local.getMeta("IsCefOpen")) return;
    alt.Player.local.setMeta("IsCefOpen", state);
});

alt.on("Client:HUD:ToggleHUD", (state) => {
    hudBrowser.emit("Client:ToggleHUD", state);
});