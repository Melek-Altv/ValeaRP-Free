    ...
    public class Main
    {
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                int rnd = new Random().Next(250, 550);
                if (!CharactersBank.HasCharacterBankMainKonto(charId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, $"Dein Gehalt i.H.v. {rnd}$ konnte nicht überwiesen werden da du kein Hauptkonto hast."); 
                    return; 
                }
                ...
                HUDHandler.SendNotification(player, 2, 7000, $"Du hast den Minijob erfolgreich abgeschlossen. Dein Gehalt i.H.v. {rnd}$ wurde dir auf dein Hauptkonto überwiesen.");
                return;
            }
            catch (Exception e)
            {
        ...
        private static void PlayerEnterVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                {
                    HUDHandler.SendNotification(player, 1, 7000, "Fahre zum ersten Zielort um den Müll abzuholen, dieser wurde auf deinem GPS markiert.");
            ...
            try
            {
                ...
                if(colShape == startJobShape && state)
                {
                    if (client.GetPlayerCurrentMinijob() == "Müllmann") {
                        HUDHandler.SendNotification(client, 1, 7000, "Drücke E um den Müllmann Minijob zu beenden.");
                    }
                    else if (client.GetPlayerCurrentMinijob() == "None") { 
                        HUDHandler.SendNotification(client, 1, 7000, "Drücke E um den Müllmann Minijob zu starten.");
                    }
                    else if (client.GetPlayerCurrentMinijob() != "None") {
                ...
                if(client.GetPlayerCurrentMinijobStep() == "PICKUP_TRASH" && state && !client.IsInVehicle)
                {
                    ...
                    HUDHandler.SendNotification(client, 1, 7000, "Mülltonne geleert, werfe den Müll in den Wagen.");
                ...
                else if(client.GetPlayerCurrentMinijobStep() == "THROW_TRASH_IN_VEHICLE" && state && !client.IsInVehicle)
                {
                    ...
                    if ((int)client.GetPlayerCurrentMinijobActionCount() < maxSpots)
                    {
                        ...
                        HUDHandler.SendNotification(client, 1, 10000, "Fahre zum nächsten Zielort um den Müll abzuholen, dieser wurde auf deinem GPS markiert.");
                    } 
                    else if((int)client.GetPlayerCurrentMinijobActionCount() >= maxSpots)
                    {
                        //zurueck zum Depot
                        if (personalThrowCol != null && personalThrowCol.Exists) personalThrowCol.Remove();
                        HUDHandler.SendNotification(client, 1, 10000, "Alles aufgesammelt mein Jung. Zurück zur Mülldeponie - das Zeug abgeben, stell das Fahrzeug einfach dort ab wo du es bekommen hast.");
        ...
        internal static void StartMinijob(IPlayer player)
        {
            try
            {
                ...
                if(player.GetPlayerCurrentMinijob() == "Müllmann")
                {
                    ...
                    if (personalThrowCol != null && personalThrowCol.Exists) personalThrowCol.Remove();
                    HUDHandler.SendNotification(player, 2, 7000, "Du hast den Minijob: Müllmann beendet.");
                    player.SetPlayerCurrentMinijob("None");
                    player.SetPlayerCurrentMinijobStep("None");
                    player.SetPlayerCurrentMinijobRouteId(0);
                    player.SetPlayerCurrentMinijobActionCount(0);
                ...
                else if(player.GetPlayerCurrentMinijob() == "None")
                {
                    ...
                    foreach(var veh in Alt.GetAllVehicles().ToList())
                    {
                        ...
                        if(veh.Position.IsInRange(Constants.Positions.Minijob_Müllmann_VehOutPos, 5f)) { 
                            HUDHandler.SendNotification(player, 3, 7000, "Der Ausparkpunkt ist belegt."); 
                            return; 
                        }
                    }
                    ServerVehicles.CreateVehicle(3039269212, charId, 2, 0, false, 0, Constants.Positions.Minijob_Müllmann_VehOutPos, Constants.Positions.Minijob_Müllmann_VehOutRot, $"MM-{charId}", 255, 255, 255, false);
                    ...
                    HUDHandler.SendNotification(player, 1, 7000, "Du hast den Minijob begonnen. Wir haben dir ein Fahrzeug zur Verfügung gestellt, steige in dies ein um zu beginnen.");
                    return;
                }
            }
            catch (Exception e)
