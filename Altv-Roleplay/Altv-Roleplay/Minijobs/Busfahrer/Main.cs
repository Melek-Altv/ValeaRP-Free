        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                if (!CharactersBank.HasCharacterBankMainKonto(charId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, $"Dein Gehalt i.H.v. {givenMoney}$ konnte nicht überwiesen werden da du kein Hauptkonto hast. Du hast aber {givenEXP}EXP erhalten (du hast nun: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP)."); 
                    return; 
                }
                ...
                HUDHandler.SendNotification(player, 2, 7000, $"Du hast den Minijob erfolgreich abgeschlossen. Dein Gehalt i.H.v. {givenMoney}$ wurde dir auf dein Hauptkonto überwiesen. Du hast {givenEXP} erhalten (deine EXP: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")})");
                return;
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (colShape == startJobShape && state)
                {
                    if (client.GetPlayerCurrentMinijob() == "Busfahrer") {
                        HUDHandler.SendNotification(client, 1, 7000, "Drücke E um den Busfahrer Minijob zu beenden.");
                    }
                    else if (client.GetPlayerCurrentMinijob() == "None") {
                        HUDHandler.SendNotification(client, 1, 7000, "Drücke E um den Busfahrer Minijob zu starten.");
                    }
                    else if (client.GetPlayerCurrentMinijob() != "None") { 
        ...
        internal static void TryStartMinijob(IPlayer player)
        {
            try
            {
                ...
                if (player.GetPlayerCurrentMinijob() == "Busfahrer")
                {
                    ...
                    HUDHandler.SendNotification(player, 2, 7000, "Du hast den Minijob: Busfahrer beendet.");
                    player.SetPlayerCurrentMinijob("None");
                    player.SetPlayerCurrentMinijobRouteId(0);
                    player.SetPlayerCurrentMinijobStep("None");
                    player.SetPlayerCurrentMinijobActionCount(0);
        ...
        public static void StartMiniJob(IPlayer player, int routeId)
        {
            try
            {
                ...
                if (CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer") < Model.GetRouteNeededEXP(routeId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, $"Du hast nicht die benötigen EXP für diese Linie ({Model.GetRouteNeededEXP(routeId)}EXP - du hast {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP).");
                    return;
                }
                ...
                ServerVehicles.CreateVehicle(Model.GetRouteVehicleHash(routeId), charId, 2, 0, false, 0, Constants.Positions.Minijob_Busdriver_VehOutPos, Constants.Positions.Minijob_Busdriver_VehOutRot, $"BUS-{charId}", 255, 255, 255, false);
                ...
                HUDHandler.SendNotification(player, 1, 7000, "Du hast den Minijob begonnen. Wir haben dir einen Bus am Tor ausgeparkt, steige ein.");
                return;
            }
            catch (Exception e)
            {
