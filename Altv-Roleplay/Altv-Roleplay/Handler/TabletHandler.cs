    ...
    class TabletHandler : IScript
    {
        ...
        public static void OpenCEF(IPlayer player)
        {
            try
            {
                ...
                if (!CharactersInventory.ExistCharacterItem(charId, "Tablet", "inventory") && !CharactersInventory.ExistCharacterItem(charId, "Tablet", "backpack")) { 
        ...
        public static void RefreshTabletData(IPlayer player, bool openTablet)
        {
            try
            {
                ...
                {
                    Global.mGlobal.VirtualAPI.TriggerClientEventSafe(player, "Client:Tablet:SetLifeinaderAds", LifeinvaderHandler.GetAds());
                }
        ...
        public static void EventsAppNewEntry(IPlayer player, string title, string callNumber, string eventDate, string eventTime, string location, string eventType, string information)
        {
            try
            {
                ...
                if (CharactersBank.GetBankAccountMoney(ownAccountNumber) < 250) { 
                    HUDHandler.SendNotification(player, 3, 5000, $"Du hast nicht genügend Geld auf deinem Konto (80$)."); 
                    return; 
                }
                CharactersBank.SetBankAccountMoney(ownAccountNumber, (CharactersBank.GetBankAccountMoney(ownAccountNumber) - 80));
                DateTime dateTime = DateTime.Now;
                ServerBankPapers.CreateNewBankPaper(ownAccountNumber, dateTime.ToString("dd.MM.yyyy"), dateTime.ToString("HH.mm"), "Ausgehende Überweisung", $"Event Dienstleister", $"Event-Eintrag: {eventDate}", $"-80$", "Online Banking");
                CharactersTablet.CreateServerTabletEvent(charId, title, callNumber, eventDate, eventTime, location, eventType, information);
                HUDHandler.SendNotification(player, 2, 5000, $"Sie haben erfolgreich ein Event für eine Gebühr von 80$ eingetragen. Die Anzeigedauer beträgt 7 Tage.");
                RefreshTabletData(player, false);
        ...
        public void VehicleStoreBuyVehicle(IPlayer player, string hash, int shopId, string color)
        {
            try
            {
                ...
                ServerVehicles.CreateVehicle(fHash, charId, 0, 0, true, 1, new Position(0, 0, 0), new Rotation(0, 0, 0), $"NL{rnd}", color_R, color_G, color_B, false);
        ...
        public static void DeleteFactionDispatch(IPlayer player, int factionId, int senderId)
        {
            try
            {
                ...
                string dispatches = ServerFactions.GetFactionDispatches(factionId);
                foreach (var client in Alt.GetAllPlayers().Where(x => x.Exists && ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == factionId))
                {
                    Global.mGlobal.VirtualAPI.TriggerClientEventSafe(client, "Client:Tablet:setDispatches", factionId, dispatches);
                }
        ...
        public static void CompanyAppInviteNewMember(IPlayer player, string targetCharName, int companyId)
        {
            try
            {
                ...
                if(targetCharId <= 0) { HUDHandler.SendNotification(player, 4, 5000, "Ein unerwarteter Fehler ist aufgetreten [COMPANY-INVITE-002]"); 
                    return; 
                }
                ...
                if(targetPlayer == null || !targetPlayer.Exists) { 
                    HUDHandler.SendNotification(player, 4, 5000, "Der angegebene Spieler ist nicht in deiner Nähe oder ihr seid nicht an dem Verwaltungspunkt des Unternehmens. [FEHLER: COMPANY-INVITE-003]"); 
                    return; 
                }
                if(!player.Position.IsInRange(ServerCompanys.GetServerCompanyPosition(companyId), 5f) || !targetPlayer.Position.IsInRange(ServerCompanys.GetServerCompanyPosition(companyId), 5f)) { 
                    HUDHandler.SendNotification(player, 3, 5000, "Der angegebene Spieler ist nicht in deiner Nähe oder ihr seid nicht an dem Verwaltungspunkt des Unternehmens. [FEHLER: COMPANY-INVITE-004]"); 
                    return; 
                }
        ...
        public static void FactionManagerAppInviteNewMember(IPlayer player, string targetCharName, int dienstnummer, int factionId)
        {
            try
            {
                ...
                int targetCharId = Characters.GetCharacterIdFromCharName(targetCharName);
                if(targetCharId <= 0) { HUDHandler.SendNotification(player, 4, 5000, "Ein unerwarteter Fehler ist aufgetreten [FACTION-INVITE-002]"); 
                    return;
                }
                ...
                var targetPlayer = Alt.GetAllPlayers().ToList().FirstOrDefault(x => x.GetCharacterMetaId() == (ulong)targetCharId);
                if(targetPlayer == null || !targetPlayer.Exists) { 
                    HUDHandler.SendNotification(player, 4, 5000, "Der angegebene Spieler ist nicht in deiner Nähe oder ihr seid nicht an dem Verwaltungspunkt der Fraktion. [FEHLER: FACTION-INVITE-003]");
                    return; 
                }
                if(!player.Position.IsInRange(targetPlayer.Position, 5f)) { 
                    HUDHandler.SendNotification(player, 3, 5000, "Der angegebene Spieler ist nicht in deiner Nähe oder ihr seid nicht an dem Verwaltungspunkt der Fraktion. [FEHLER: FACTION-INVITE-004]");
                    return; 
                }
        ...
        public static void CompanyAppLeaveCompany(IPlayer player)
        {
            try
            {
                ...
                if(ServerCompanys.GetCharacterServerCompanyRank(charId) > 1) {
                    HUDHandler.SendNotification(player, 4, 5000, "Du kannst als Geschäftsführer dein Unternehmen nicht einfach verlassen, tue dies beim Gewerbeamt.");
                    return;
                }
        ...
        public static void CompanyAppRankAction(IPlayer player, int rankId, int targetCharId)
        {
            try
            {
                ...
                if (charId <= 0) return;
                if(!ServerCompanys.IsCharacterInAnyServerCompany(charId)) {
                    HUDHandler.SendNotification(player, 4, 7000, "Du bist in keinem Unternehmen welches du verwalten kannst."); 
                    return;
                }
                int companyId = ServerCompanys.GetCharacterServerCompanyId(charId);
                if(companyId <= 0) { HUDHandler.SendNotification(player, 4, 7000, "Ein unerwarteter Fehler ist aufgetreten. [COMPANY-RANK-ACTION-001]");
                    return;
                }
                if(ServerCompanys.GetCharacterServerCompanyRank(charId) < 1) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Dafür hast du keine Berechtigung.");
                    return; 
                }
                if(!ServerCompanys.IsCharacterInAnyServerCompany(targetCharId)) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Ein unerwarteter Fehler ist aufgetreten. [COMPANY-RANK-ACTION-002]");
                    return;
                }
                if(ServerCompanys.GetCharacterServerCompanyRank(targetCharId) > 1) {
                    HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht verwalten."); 
                    return;
                }
                if(ServerCompanys.GetCharacterServerCompanyRank(charId) == 1 && ServerCompanys.GetCharacterServerCompanyRank(targetCharId) >= 1) { 
                    HUDHandler.SendNotification(player, 3, 7000, "Dieser Person kannst du diesen Rang nicht setzen."); 
                    return; 
                }
                if(ServerCompanys.GetCharacterServerCompanyRank(targetCharId) == rankId) { HUDHandler.SendNotification(player, 3, 7000, "Die Person besitzt diesen Rang bereits."); 
                    return; 
                }
                if(ServerCompanys.GetCharacterServerCompanyRank(charId) == 1 && ServerCompanys.GetCharacterServerCompanyRank(targetCharId) == 0 && (rankId > 0 && rankId < 5)) { 
                ...
                if (rankId == 1337)
                {
                    ...
                    RefreshTabletData(player, false);
                    HUDHandler.SendNotification(player, 2, 7000, $"Du hast die Person {Characters.GetCharacterName(targetCharId)} aus deinem Unternehmen entlassen.");
                    ...
                    if (targetPlayer != null && targetPlayer.Exists)
                    {
                        ...
                        HUDHandler.SendNotification(targetPlayer, 1, 7000, $"Du wurdest aus dem Unternehmen {ServerCompanys.GetServerCompanyName(companyId)} entlassen.");
                    }
                    return;
                }
                ...
                RefreshTabletData(player, false);
                HUDHandler.SendNotification(player, 2, 7000, $"Du hast der Person {Characters.GetCharacterName(targetCharId)} den Rang {ServerCompanys.GetServerCompanyRankName(rankId)} gesetzt.");
                ...
                {
                    RefreshTabletData(targetPlayer, false);
                    HUDHandler.SendNotification(targetPlayer, 1, 7000, $"Im Unternehmen '{ServerCompanys.GetServerCompanyName(companyId)}' wurde dir der Rang {ServerCompanys.GetServerCompanyRankName(rankId)} gesetzt.");
                }
            }
            catch(Exception e)
            {
        ...
        public static void FactionManagerRankAction(IPlayer player, string action, int targetCharId)
        {
            ...
            {
                if (player == null || !player.Exists || targetCharId <= 0) return;
                if (action != "rankup" && action != "rankdown" && action != "remove") { HUDHandler.SendNotification(player, 3, 7000, "Ein unerwarteter Fehler ist aufgetreten. [FACTION-RANKACTION-001]"); return; }
                int charId = User.GetPlayerOnline(player);
                if (charId <= 0) return;
                if(charId == targetCharId) { HUDHandler.SendNotification(player, 3, 7000, "Du kannst dich nicht selbst verwalten."); 
                    return; 
                }
                if (!ServerFactions.IsCharacterInAnyFaction(charId)) {
                    HUDHandler.SendNotification(player, 4, 7000, "Du bist in keiner Fraktion um dies zu tun."); 
                    return; 
                }
                if (!ServerFactions.IsCharacterInAnyFaction(targetCharId)) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Die ausgewählte Person ist in keiner Fraktion dessen Rang du verwalten kannst.");
                    return; 
                }
                ...
                if(ServerFactions.GetCharacterFactionRank(charId) < ServerFactions.GetFactionMaxRankCount(ServerFactions.GetCharacterFactionId(charId)) - 2) { 
                    HUDHandler.SendNotification(player, 3, 7000, "Du bist nicht dazu berechtigt."); 
                    return; 
                }
                var targetPlayer = Alt.GetAllPlayers().ToList().FirstOrDefault(x => x.GetCharacterMetaId() == (ulong)targetCharId);
                ...
                if (action == "remove")
                {
                    if(currentActorRank <= currentTargetRank) { 
                        HUDHandler.SendNotification(player, 3, 7000, "Dazu hast du keine Berechtigung."); 
                        return; 
                    }
                    ServerFactions.RemoveServerFactionMember(factionId, targetCharId);
                    HUDHandler.SendNotification(player, 2, 7000, $"Du hast die Person {Characters.GetCharacterName(targetCharId)} erfolgreich aus der Fraktion {ServerFactions.GetFactionFullName(factionId)} entlassen.");
                    RefreshTabletData(player, false);
                    if (targetPlayer == null || !targetPlayer.Exists) return;
                    HUDHandler.SendNotification(targetPlayer, 1, 7000, $"Du wurdest aus der Fraktion {ServerFactions.GetFactionFullName(factionId)} entlassen.");
                ...
                {                    
                    if(currentTargetRank + 1 >= ServerFactions.GetFactionMaxRankCount(factionId)) { 
                        HUDHandler.SendNotification(player, 3, 7000, "Auf den Rang der Fraktionsleiter kann nur das Bürgeramt befördern."); 
                        return; 
                    }
                    if(currentTargetRank + 1 >= currentActorRank) {
                        HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht weiter befördern"); 
                        return; 
                    }
                    if(currentTargetRank == ServerFactions.GetFactionMaxRankCount(factionId)) { 
                        HUDHandler.SendNotification(player, 3, 7000, "Diesen Rang gibt es nicht."); 
                        return; 
                    }
                    if (currentActorRank <= currentTargetRank) { 
                        HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht verwalten."); 
                    ...
                    if (currentActorRank == ServerFactions.GetFactionMaxRankCount(factionId) - 1 && currentTargetRank >= ServerFactions.GetFactionMaxRankCount(factionId) - 1) { 
                        HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht verwalten."); 
                        return; 
                    }
                    if(currentActorRank == ServerFactions.GetFactionMaxRankCount(factionId) - 2 && currentTargetRank >= ServerFactions.GetFactionMaxRankCount(factionId) - 2) { 
                        HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht verwalten."); 
                        return; 
                    }
                    ServerFactions.SetCharacterFactionRank(targetCharId, currentTargetRank + 1);
                    HUDHandler.SendNotification(player, 2, 7000, $"Sie haben den Rang von der Person {Characters.GetCharacterName(targetCharId)} von {currentTargetRank} auf {currentTargetRank + 1} geändert.");
                    RefreshTabletData(player, false);
                    if (targetPlayer == null || !targetPlayer.Exists) return;
                    HUDHandler.SendNotification(targetPlayer, 1, 7000, $"Ihr Rang in der Fraktion {ServerFactions.GetFactionFullName(factionId)} wurde von {currentTargetRank} auf {currentTargetRank + 1} geändert.");
                ...
                else if(action == "rankdown")
                {
                    if(currentTargetRank - 1 <= 0) { 
                        HUDHandler.SendNotification(player, 4, 7000, "Weiter runter geht es nicht.");
                        return; 
                    }
                    if(currentActorRank <= currentTargetRank) { HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht verwalten.");
                        return; 
                    }
                    if(currentActorRank == ServerFactions.GetFactionMaxRankCount(factionId) - 1 && currentTargetRank >= ServerFactions.GetFactionMaxRankCount(factionId) - 1) {
                        HUDHandler.SendNotification(player, 3, 7000, "Diese Person kannst du nicht verwalten."); 
        ...
        public static void FactionManagerSetRankPaycheck(IPlayer player, int rankId, int paycheck)
        {
            try
            {
                ...
                if (paycheck > 650) { HUDHandler.SendNotification(player, 3, 7000, "Fehler: Das Gehalt kann nur maximal auf 650$ gesetzt werden."); 
                    return;
                }
                int charId = User.GetPlayerOnline(player);
                if (charId <= 0) return;
                if (!ServerFactions.IsCharacterInAnyFaction(charId)) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Du bist in keiner Fraktion dessen Gehalt du festlegen kannst.");
                    return; 
                }
                int factionId = ServerFactions.GetCharacterFactionId(charId);
                if (factionId <= 0) return;
                ...
                if (charRank < ServerFactions.GetFactionMaxRankCount(factionId) - 2) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Dazu hast du keine Berechtigungen."); 
                    return;
                }
                if (!ServerFactions.ExistServerFactionRankOnId(factionId, rankId)) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Ein unerwarteter Fehler ist aufgetreten. [FACTION-RANKPAYCHECK-001]");
                    return;
                }
                ServerFactions.SetFactionRankPaycheck(factionId, rankId, paycheck);
                HUDHandler.SendNotification(player, 2, 7000, $"Du hast das Gehalt des Ranges '{ServerFactions.GetFactionRankName(factionId, rankId)}' der Fraktion '{ServerFactions.GetFactionFullName(factionId)}' von {currentPaycheck}$ auf {paycheck}$ gesetzt.");
                RefreshTabletData(player, false);
        ...
        public static void SendDispatchToFaction(IPlayer player, int factionId, string msg)
        {
            try
            {
                ...
                if(player.HasPlayerHandcuffs() || player.HasPlayerRopeCuffs()) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Fehler: Du bist gefesselt."); 
                    return; 
                }
                if (ServerFactions.ExistDispatchBySender(charId, factionId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, "Du hast bereits einen Dispatch offen."); 
                    return;
                }
                ServerFactions.AddNewFactionDispatch(charId, factionId, msg, player.Position);
                HUDHandler.SendNotification(player, 2, 7000, "Notruf erfolgreich gesendet.");
        ...
        [AsyncClientEvent("Server:Tablet:InternetAppSearchVehicle")]
        public static void LSPDAppSearchPerson(IPlayer player, string vehicleName)
        {
            try
            {
                if (player == null || !player.Exists || vehicleName == "") return;
                ...
                if (!ServerAllVehicles.ExistVehicleName(vehicleName)) { HUDHandler.SendNotification(player, 3, 5000, "Fehler: Der eingegebene Fahrzeugname wurde nicht gefunden."); return; }
                long hash = ServerAllVehicles.GetVehicleHashOnName(vehicleName);
                if (hash == 0) return;
                string Price = Convert.ToString(ServerAllVehicles.GetVehiclePrice(hash));
                string manufactur = ServerAllVehicles.GetVehicleManufactur(hash);
                string category = ServerAllVehicles.GetVehicleCategory(hash);
                string trunkcapacity = Convert.ToString(ServerAllVehicles.GetVehicleTrunkCapacity(hash));
                string maxfuel = Convert.ToString(ServerAllVehicles.GetVehicleMaxFuel(hash));
                string fueltype = ServerAllVehicles.GetVehicleFuelType(hash);
                string seats = Convert.ToString(ServerAllVehicles.GetVehicleSeats(hash));
                string tax = Convert.ToString(ServerAllVehicles.GetVehicleTaxes(hash));
                player.EmitLocked("Client:Tablet:SetInternetAppVehicleSearchData", Price, manufactur, category, trunkcapacity, maxfuel, fueltype, seats, tax);
                HUDHandler.SendNotification(player, 2, 4000, $"Fahrzeugabfrage durchgeführt: {vehicleName}.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
