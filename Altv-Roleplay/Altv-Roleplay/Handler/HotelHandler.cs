    ...
    class HotelHandler : IScript
    {
        ...
        public static void RentHotel(IPlayer player, int hotelId, int apartmentId)
        {
            try
            {
                ...
                if (ServerHotels.HasCharacterAnApartment(charId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, $"Du besitzt bereits ein Hotelzimmer in dem Hotel '{ServerHotels.GetCharacterRentedHotelName(charId)}'."); 
                    return; 
                }
                if (ServerHotels.GetApartmentOwner(hotelId, apartmentId) > 0) { 
                    HUDHandler.SendNotification(player, 3, 7000, "Dieses Apartment ist bereits vermietet."); 
                    return; 
                }
                ...
                if(CharactersBank.GetBankAccountMoney(accNumber) < ServerHotels.GetApartmentPrice(hotelId, apartmentId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, $"Soviel Geld hast du auf deinem Konto nicht ({ServerHotels.GetApartmentPrice(hotelId, apartmentId)}$) - du hast {CharactersBank.GetBankAccountMoney(accNumber)}$"); 
                    return; 
                }
                ...
                HUDHandler.SendNotification(player, 2, 7000, $"Sie haben sich das Zimmer mit der Zimmernummer '{apartmentId}' erfolgreich gemietet (Kosten: {ServerHotels.GetApartmentPrice(hotelId, apartmentId)}$). Dieses Zimmer läuft automatisch nach {ServerHotels.GetApartmentRentHours(hotelId, apartmentId)} Stunden ab.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public static void LockHotel(IPlayer player, int hotelId, int apartmentId)
        {
            try
            {
                ...
                if (!ServerHotels.HasCharacterAnApartment(charId)) { 
                    HUDHandler.SendNotification(player, 3, 7000, $"Du besitzt hier kein Zimmer."); 
                    return; 
                }
                ...
                if (hotel.isLocked) HUDHandler.SendNotification(player, 2, 7000, $"Du hast dein Zimmer aufgeschlossen");
                else HUDHandler.SendNotification(player, 4, 7000, $"Du hast dein Zimmer abgeschlossen");
        ...
        public static void EnterHotel(IPlayer player, int hotelId, int apartmentId)
        {
            try
            {
                ...
                if(ServerHotels.GetApartmentOwner(hotelId, apartmentId) <= 0) { 
                    HUDHandler.SendNotification(player, 3, 7000, "Fehler: Dieses Zimmer ist nicht vermietet."); 
                    return; 
                }
                var hotel = ServerHotels.ServerHotelsApartments_.FirstOrDefault(x => x.hotelId == hotelId && x.id == apartmentId);
                if (hotel == null) return;
                ...
                if(hotel.isLocked) { 
                    HUDHandler.SendNotification(player, 3, 7000, "Fehler: Das Zimmer ist abgeschlossen."); 
                    return; 
                }
        ...
        public static void StorageHotelItem(IPlayer player, int apartmentId, string itemName, int itemAmount, string fromContainer)
        {
            try
            {
                ...
                if(ServerHotels.GetHotelStorageItemWeight(apartmentId) >= 15f || (ServerHotels.GetHotelStorageItemWeight(apartmentId) + itemWeight) >= 15f) {
                    HUDHandler.SendNotification(player, 3, 7000, "Fehler: Soviel passt in das Lager nicht rein (maximal 15kg Lagerplatz).");
                    return;
                }
                ...
                DiscordLog.SendEmbed("hotelStorage", "hotelStorage Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat {itemAmount}x {itemName} in das Lager gelegt.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public static void TakeHotelItem(IPlayer player, int apartmentId, string itemName, int itemAmount)
        {
            try
            {
                ...
                if(!ServerHotels.ExistServerHotelStorageItem(apartmentId, itemName)) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Fehler: Der Gegenstand existiert im Lager nicht."); 
                    return; 
                }
                if (ServerHotels.GetServerHotelStorageItemAmount(apartmentId, itemName) < itemAmount) { 
                    HUDHandler.SendNotification(player, 4, 7000, "Fehler: Soviele Gegenstände sind nicht im Lager."); 
                    return; 
                }
                float itemWeight = ServerItems.GetItemWeight(itemName) * itemAmount;
                ...
                DiscordLog.SendEmbed("hotelStorage", "hotelStorage Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat {itemAmount}x {itemName} aus dem Lager genommen.");
                ...
                {
                    HUDHandler.SendNotification(player, 2, 7000, $"Du hast {itemName} ({itemAmount}x) aus deinem Lager genommen (Lagerort: Inventar).");
                ...
                {
                    HUDHandler.SendNotification(player, 2, 7000, $"Du hast {itemName} ({itemAmount}x) aus deinem Lager genommen (Lagerort: Rucksack / Tasche).");
