    ...
    class CharactersTablet
    {
        ...
        public static string GetCharacterTabletApps(int charId)
        {
            ...
            var items = CharactersTabletApps_.Where(x => x.charId == charId).Select(x => new
            {
                ...
                dmvapp = ServerFactions.IsCharacterInAnyFaction(charId) && ServerFactions.IsCharacterInFactionDuty(charId) && ServerFactions.GetCharacterFactionId(charId) == 5,
            }).ToList();
        ...
        {
            var items = ServerTabletEventsData_.Where(x => DateTime.Now.Subtract(Convert.ToDateTime(x.created)).TotalHours < 48).Select(x => new
            {
                x.id,
                x.title,
                owner = x.ownerName,
        ...
        public static bool GetGarageState(int vehId)
        {
            try
            {
                ...
                if (vehicle != null)
                {
                    if (vehicle.isInHouseGarage || vehicle.isInGarage) { return true; }
                    else if (!vehicle.isInHouseGarage || !vehicle.isInGarage) { return false; }
                }
            }
            catch (Exception e)
            {
        ...
        public static Position GetCharacterTabletVehiclePosition(int vehId)
        {
            try
            {
                ...
                if (vehicle != null)
                {
                    ...
                    else if (vehicle.isInHouseGarage) { return ServerHouses.GetHouseGaragePosition(vehicle.garageId); }
                    else if (!vehicle.isInGarage && !vehicle.isInHouseGarage) { return new Position(vehicle.posX, vehicle.posY, vehicle.posZ); }
                }
            }
            catch (Exception e)
            {
