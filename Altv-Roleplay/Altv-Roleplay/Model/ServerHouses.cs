...
namespace Altv_Roleplay.Model
{
    class ServerHouses
    {
        ...
        internal static void CreateHouse(int id, int interiorId, int ownerId, string street, int price, int maxRenters, int rentPrice, bool isRentable, bool hasStorage, bool hasAlarm, bool hasBank, Position entrancePos, int money, int garageSlots, Position garagePos, Position garageVehPos, Rotation garageVehRot)
        {
            try
            {
                var houseData = new Server_Houses
                {
                    ...
                    garagePos = garagePos,
                    garageVehPos = garageVehPos,
                    garageVehRot = garageVehRot
                };
                ...
                EntityStreamer.MarkerStreamer.Create(EntityStreamer.MarkerTypes.MarkerTypeUpsideDownCone, garagePos, new System.Numerics.Vector3(0.5f, 0.5f, 0.5f), color: new Rgba(150, 50, 50, 125));
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public static Position GetHousePosition(int houseId)
        {
        ...
        public static void SetHouseInformations(int houseId)
        {
            try
            {
                if (houseId <= 0) return;
                ...
                if (house != null)
                {
                    ...
                    house.hasStorage = false;
                    house.hasAlarm = false;
                    house.hasBank = false;
                    using (gtaContext db = new gtaContext())
                    {
                        db.Server_Houses.Update(house);
        ...
        public static void ClearHouseInventory(int houseId)
        {
            try
            {
                if (houseId <= 0) return;
                ...
                {
                    foreach (var entry in db.Server_Houses_Storages.ToList().Where(x => x.houseId == houseId))
                        db.Server_Houses_Storages.Remove(entry);
                    db.SaveChanges();
                }
            }
            catch (Exception e)
        ...
        public static void ClearHouseRenters(int houseId)
        {
            try
            {
                if (houseId <= 0) return;
                using (var db = new gtaContext())
                {
                    foreach (var entry in db.Server_Houses_Renters.ToList().Where(x => x.houseId == houseId))
                        db.Server_Houses_Renters.Remove(entry);
                    db.SaveChanges();
                }
            }
            catch (Exception e)
        ...
        public static int GetHouseRenters(int houseId, int renterId)
        {
            try
            {
                if (houseId <= 0 || renterId <= 0) return 0;
                var renter = ServerHousesRenter_.FirstOrDefault(i => i.houseId == houseId && i.charId == renterId);
                if (renter != null) return renter.charId;
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public static Position GetHouseGaragePosition(int garageid)
        {
            Position pos = new Position(0, 0, 0);
            if (garageid == 0) return pos;
            var garage = ServerHouses_.FirstOrDefault(x => x.id == garageid);
            if (garage != null)
            {
                pos = garage.garagePos;
            }
            return pos;
        }
    }
