...
namespace Altv_Roleplay.Model
{
    class User
    {
        ...
        public static int GetPlayerMaxCharacters(IPlayer player)
        {
        ...
        public static bool ExistPlayerByCharacterId(int charId)
        {
            try
            {
                if (charId <= 0) return false;
                var pl = Characters.PlayerCharacters.ToList().FirstOrDefault(x => x.charId == charId);
                if (pl != null) return true;
            }
            catch (Exception e)
            {
        ...
        public static void SetPlayerICWhitelisted(int charId, bool state)
        {
            try
            {
                if (charId <= 0) return;
        ...
        public static int GetPlayerCharId(ClassicPlayer player)
        {
            if (player == null || !player.Exists) return 0;
            var pl = Characters.PlayerCharacters.ToList().FirstOrDefault(x => x.charId == player.CharacterId);
