...
namespace Altv_Roleplay.Model
{
    class ServerVehicles
    {
        ...
        public static void CreateServerVehicle(int id, int charid, uint hash, int vehType, int factionid, float fuel, float km, bool engineState, bool isEngineHealthy, bool lockState, bool isInGarage, int garageId, Position position, Rotation rotation, string plate, DateTime lastUsage, DateTime buyDate, bool isInHouseGarage)
        {
            var newVehicleData = new Server_Vehicles
            {
                ...
                lastUsage = lastUsage,
                buyDate = buyDate,
                isInHouseGarage = isInHouseGarage
            };
            ServerVehicles_.Add(newVehicleData);

            if (isInGarage || isInHouseGarage) return;
            IVehicle veh = Alt.CreateVehicle(hash, position, rotation);
            if (position == new Position(0, 0, 0)) { SetVehicleInGarage(veh, true, 10, false); return; }
            if (lockState == true) { veh.LockState = VehicleLockState.Locked; }
            else if (lockState == false) { veh.LockState = VehicleLockState.Unlocked; }
            if (plate.StartsWith("NL") && !plate.Contains("LSPD") && !plate.Contains("LSMD") && !plate.Contains("BNY") && !plate.Contains("DMV") && !plate.Contains("VUC") && !plate.Contains("DOJ"))
            {
        ...
        internal static void UpdateVehicleLastPosition(IVehicle veh)
        {
            try
            {
                if (veh == null || !veh.Exists || veh.GetVehicleId() <= 0) return;
                Server_Vehicles vehicle = ServerVehicles_.FirstOrDefault(x => x.id == veh.GetVehicleId());
                if (vehicle == null) return;
                Position distance = new Position(veh.Position.X, veh.Position.Y, veh.Position.Z);
                float fKM = distance.Distance(new Position(vehicle.posX, vehicle.posY, vehicle.posZ)) / 1000;
                fKM = fKM + ServerVehicles.GetVehicleKM(veh);
                ServerVehicles.SetVehicleKM(veh, fKM);
                vehicle.posX = veh.Position.X;
                vehicle.posY = veh.Position.Y;
                vehicle.posZ = veh.Position.Z;
                vehicle.rotX = veh.Rotation.Roll;
                vehicle.rotY = veh.Rotation.Pitch;
                vehicle.rotZ = veh.Rotation.Yaw;
                vehicle.KM = GetVehicleKM(veh);
        ...
        public static void SetVehicleInGarage(IVehicle veh, bool isIngarage, int garageId, bool isInHouseGarage)
        {
            try
            {
                if (veh == null || !veh.Exists) return;
                ...
                dbVehicle.isInGarage = isIngarage;
                dbVehicle.isInHouseGarage = isInHouseGarage;
                dbVehicle.lastUsage = DateTime.Now;
                if (isIngarage == true || isInHouseGarage == true) { dbVehicle.garageId = garageId; veh.Remove(); }
                using (gtaContext db = new gtaContext())
                {
        ...
        public static void CreateVehicle(long hash, int charid, int vehtype, int faction, bool isInGarage, int garageId, Position pos, Rotation rot, string plate, int colorR, int colorG, int colorB, bool isInHouseGarage)
        {
            try
            {
        ...
        public static string GetVehiclePlateByOwner(long vehId)
        {
            try
            {
                var vehicle = ServerVehicles_.FirstOrDefault(x => x.id == vehId);
        ...
        public static int GetVehicleType2(long vehId)
        {
            if (vehId == 0) return -1;
            var vehs = ServerVehicles_.FirstOrDefault(v => v.id == vehId);
            if (vehs != null)
        ...
        public static int GetVehicleID(int charId)
        {
            if (charId <= 0) return 0;
            var vehs = ServerVehicles_.FirstOrDefault(v => v.charid == charId);
            if (vehs != null)
            {
                return vehs.id;
            }
