    ...
    class ServerItems
    {
        ...
        public static string ReturnNormalItemName(string itemName)
        {
            try
            {
                ...
                else if(itemName.Contains("Kaufvertrag")) { normalName = "Kaufvertrag"; }
                else if(itemName.Contains("Generalschluessel")) { normalName = "Generalschluessel"; }
                else if(itemName.Contains("Hausschluessel")) { normalName = "Hausschluessel"; }
                return normalName;
            }
            catch(Exception e)
            {
        ...
        public static int GetAttachmentId(string itemName)
        {
            itemName = ReturnNormalItemName(itemName);
            var item = ServerItems_.ToList().FirstOrDefault(x => x.itemName == itemName);
            if (item != null) return item.attachmentId;
            return 0;
        }
